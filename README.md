...
| `ai-memory dream-generate` | 从会话事件生成 Dream（梦境） |
| `ai-memory dream-for-session` | 为特定会话生成 Dream |
| `ai-memory brainstorming-sync` | 同步 Brainstorming 层到本地 |
| `ai-memory import` | 从 ZIP 导入数据 |
...
See [docs/FAQ.md](docs/FAQ.md) for Codex MCP setup and generated config examples.
